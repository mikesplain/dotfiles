...
in
{
  programs.zsh = {
    ...
    initContent = ''
                  ${optionalString platform.isDarwin ''
      ...
      ${optionalString platform.isDarwin "        switch                  darwin-rebuild switch --flake .\n"}${optionalString platform.isLinux "        switch                  home-manager switch --flake .\n"}
              update_flake_from_pr    Approve + merge latest successful flake update PR and pull
              w                       windsurf .
              c                       cursor .
      ${optionalString platform.isDarwin "        clear_dns_cache         Flush macOS DNS cache\n"}
            ...
            EOF
                  ...
                  ${optionalString platform.isDarwin "export SSH_AUTH_SOCK=~/Library/Group\\ Containers/2BUA8C4S2C.com.1password/t/agent.sock\n"}
                  ...
                  ''}
                  ${optionalString platform.isLinux ''
                    switch() {
                  ...
                  enable_k9s_node_shell() {
                    local cluster_dir="${k9sClusterDir}"
